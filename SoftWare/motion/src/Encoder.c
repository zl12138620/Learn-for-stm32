...
/* ========================================================================
 * SW 按键: 读当前电平(消抖后)。1 = 按着, 0 = 松着。
 *
 *   和 Encoder_SwTakePress 的分工见 Encoder.h 的注释:
 *     边沿事件 -> 本工程原来的菜单状态机("按一下切界面")
 *     电平     -> LVGL 的 encoder 设备(LVGL 自己判边沿, 但要能知道"现在还按着")
 *
 *   ⚠ 不需要进临界区: s_sw_stable 是**单个字节**, Cortex-M4 读一个字节
 *     是原子的, 不存在"读到半个值"这回事。读到的要么是这次改之前的值,
 *     要么是之后的值 —— 两种都对。真正的边沿判断在调用方那边做,
 *     和这里的采样时机差 1ms 无所谓(人手的动作远慢于 1ms)。
 * ====================================================================== */
uint8_t Encoder_SwIsDown(void)
{
  return (s_sw_stable == 0U) ? 1U : 0U;
}
