/**
  ...
  *
  *          接线: A 相 -> PB6, B 相 -> PB5, SW -> PB0 (本文件宏可改)
...
/* SW 按键(编码器轴往下按): 上拉输入, 松开=高, 按下=低。
   Encoder.c 只负责把它配成输入, 按下做什么由 main.c 轮询决定 */
#define ENC_SW_PIN          GPIO_Pin_0             /* SW = PB0 */
...
/* ============================ API ============================ */
void     Encoder_Init(void);        /* 配置 PB6/PB5/SW 输入 + EXTI6 中断 + TIM7 + NVIC */
